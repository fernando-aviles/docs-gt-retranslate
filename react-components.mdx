---
title: "React"
description: "Build interactive and reusable elements with React widgets"
icon: "react"
---

import { ColorGenerator } from "/snippets/color-generator.jsx";

[React widgets](https://react.dev) are a powerful way to create interactive and reusable elements in your documentation.

## Using React widgets

You can build React widgets directly in your `MDX` files using [React hooks](https://react.dev/reference/react/hooks).

### Example

This example declares a `Counter` widget and then uses it with `<Counter />`.
...
The counter renders as an interactive React widget.

<Counter />

## Importing widgets

You can import widgets from your `snippets` folder. Unlike regular React, you cannot import widgets from every `MDX` file. Reusable widgets must be referenced from files within the `snippets` folder. Learn more about [reusable snippets](/reusable-snippets).

### Example

This example declares a `ColorGenerator` widget that uses multiple React hooks and then uses it in an `MDX` file.
...
Import the `ColorGenerator` widget and use it in an `MDX` file:
...
The color generator renders as an interactive React widget.
...
<AccordionGroup>
  <Accordion title="Client-side rendering impact">
    React hook widgets render on the client-side, which has several implications:

    - **SEO**: Search engines might not fully index dynamic content.
    - **Initial load**: Visitors may experience a flash of loading content before widgets render.
  ...
  <Accordion title="Performance best practices">
    ...
    - **Reduce re-renders**: Break large widgets into smaller ones to prevent cascading re-renders.
    - **Lazy loading**: Consider lazy loading complex widgets to improve initial page load time.
  </Accordion>
</AccordionGroup>